...
- 가상 사용자 10명이 1분 동안 `/api/memory`를 호출합니다.
...
가상 사용자가 `5명 → 30명 → 5명 → 0명` 순서로 변하면서 `/api/cpu`를 호출합니다. CPU 포화로 응답 시간이 증가하거나 일부 요청이 실패할 수 있으므로 `http_req_duration`, `http_req_failed`와 CPU 사용량을 함께 확인합니다.
...
일반, CPU, 메모리, 지연, 오류 API를 무작위로 호출합니다. `/api/error`가 의도적으로 HTTP 500을 반환하므로 현재 혼합 테스트에서는 일부 `check` 실패가 예상됩니다. 이 실패는 서버 로그와 함께 확인해 의도된 `/api/error` 응답인지 구분해야 합니다.
...
```bash
BASE_URL=http://127.0.0.1:8010 k6 run ./k6/normal-test.js
```
