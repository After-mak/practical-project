...
class AllTasks(SequentialTaskSet):
    ...
    class AuthenticatedTasks(TaskSet):
        ...
        """
        def _wait_for_queue(self, timeout=300):
            """
            /api/waiting/status 를 0.5초마다 폴링하여 대기열 통과 여부 확인
            통과되면 True, timeout 초 초과 시 False 반환
            """
            token = self.client.cookies.get('waiting_token')
            for _ in range(timeout):
                with self.client.get(
                    f"/api/waiting/status?token={token}",
                    name="/api/waiting/status [polling]",
                    catch_response=True
                ) as resp:
                    resp.success()  # 폴링 요청은 항상 성공으로 기록
                    if resp.json().get('passed'):
                        return True
                time.sleep(0.5)
            return False  # 300초 초과시 다시 처음부터 기능 수행
        ...
        def on_start(self):
            ...
            """
            while not self._wait_for_queue(timeout=300):
                pass  # 5분 안에 통과 못하면 다시 폴링 시작 (대기열 자동 재등록)
