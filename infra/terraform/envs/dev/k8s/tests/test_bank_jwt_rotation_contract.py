...
def test_rotation_never_manages_bank_namespaces():
    ...
    assert 'data "kubernetes_namespace_v1" "bank"' in source
    assert "from = kubectl_manifest.bank_namespace" in source
    removed_block = source.split("removed {", 1)[1].split("data ", 1)[0]
    assert "destroy = false" in removed_block
    assert 'name = each.value' in source
...
def test_rotation_only_gates_secret_inputs_and_secret_resource():
    ...
    assert source.count(conditional) == 2
